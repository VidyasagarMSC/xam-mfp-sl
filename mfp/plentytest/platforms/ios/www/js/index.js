...
function callFeedAdapter() {
    WL.Logger.debug("called Feed Adapter");
    var resourceRequest = new WLResourceRequest("/adapters/FeedAdapter/getFeed",WLResourceRequest.GET);
...
function callProtectedAPI() {
    WL.Logger.debug("called protected API");
    var url = "http://192.168.1.15:3000/api/Offers";
    var request = new WLResourceRequest(url, WLResourceRequest.GET);
    request.send().then(
    function(response){
        WL.Logger.debug("received response from API");
        WL.Logger.debug(response);
    },
    function(error){
        WL.Logger.debug("received error from API" + JSON.stringify(error));
  }
);
}